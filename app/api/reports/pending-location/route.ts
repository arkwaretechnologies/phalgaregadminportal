...
export async function GET(request: NextRequest) {
  try {
    ...
    if (countBy === 'participant') {
      ...
      // First get all pending registration regids for the conference (no row limit)
      const { data: pendingRegs, error: reghError } = await fetchAllRecords(
        'regh',
        'regid, confcode',
        (query) => {
          query = query.eq('status', 'PENDING').not('regid', 'is', null);
          if (confcode) {
      ...
      if (reghError) {
        console.error('Error fetching pending registrations:', reghError);
      ...
      const regids = pendingRegs.map(r => r.regid).filter(Boolean) as string[];

      // Query participants from regd table for pending registrations (no row limit)
    ...
    // Default: Count by batch (pending registrations from regh table) - no row limit
    ...
    if (error) {
      console.error('Error fetching pending location data:', error);
      return NextResponse.json({ error: 'Failed to fetch pending location data' }, { status: 500 });
    }
