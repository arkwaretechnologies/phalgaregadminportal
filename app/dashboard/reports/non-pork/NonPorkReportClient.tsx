...
interface NonPorkReportClientProps {
  initialConferences: Conference[];
  initialConfcode: string | null;
  initialStatus: StatusFilter;
}

export default function NonPorkReportClient({
  initialConferences,
  initialConfcode,
  initialStatus,
}: NonPorkReportClientProps) {
  ...
  const [registrations, setRegistrations] = useState<NonPorkRegistration[]>([]);
  const [participants, setParticipants] = useState<NonPorkParticipant[]>([]);
  const [total, setTotal] = useState(0);
  const [totalRegistrations, setTotalRegistrations] = useState(0);
  const [totalNonPorkParticipants, setTotalNonPorkParticipants] = useState(0);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedConfcode, viewMode, searchQuery, statusFilter]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const url = new URL('/api/reports/non-pork', window.location.origin);
        ...
        if (response.ok) {
          ...
          setTotalNonPorkParticipants(data.totalNonPorkParticipants || 0);
        } else {
        ...
        console.error('Error fetching non-pork report:', err);
    ...
    fetchData();
  }, [selectedConfcode, viewMode, statusFilter]);

  const pushFilters = (confcode: string, status: StatusFilter) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set('confcode', confcode);
    params.set('status', status);
    router.push(`/dashboard/reports/non-pork?${params.toString()}`);
  };

  const handleConferenceChange = (confcode: string) => {
    setSelectedConfcode(confcode);
    pushFilters(confcode, statusFilter);
  };

  const handleStatusChange = (status: StatusFilter) => {
    setStatusFilter(status);
    if (selectedConfcode) {
      pushFilters(selectedConfcode, status);
    }
  };
    ...
    try {
      ...
      if (viewMode === 'registration') {
        ...
        const data = registrations.map((reg) => ({
          ...
          'Non Pork Count': reg.nonPorkCount,
          'Total Participants': reg.participantCount,
        ...
        ws['!cols'] = [
          { wch: 15 }, { wch: 18 }, { wch: 14 }, { wch: 16 }, { wch: 30 },
          { wch: 25 }, { wch: 25 }, { wch: 30 }, { wch: 18 }, { wch: 30 },
        ];
        XLSX.utils.book_append_sheet(wb, ws, 'Non Pork Regs');
        XLSX.writeFile(wb, `${safeConfName}_non_pork_${statusLabel}_registrations.xlsx`);
      } else {
        ...
        const ws = XLSX.utils.json_to_sheet(data);
        ws['!cols'] = [
          { wch: 30 }, { wch: 45 }, { wch: 15 }, { wch: 18 }, { wch: 10 },
          { wch: 22 }, { wch: 22 }, { wch: 18 }, { wch: 30 }, { wch: 30 },
        ];
        XLSX.utils.book_append_sheet(wb, ws, 'Non Pork Parts');
        XLSX.writeFile(wb, `${safeConfName}_non_pork_${statusLabel}_participants.xlsx`);
      }
    } catch (err: any) {
  ...
  const filteredCount =
    viewMode === 'registration' ? filteredRegistrations.length : filteredParticipants.length;
  ...
  return (
    <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 bg-clip-text text-transparent">
                Non Pork Participants Summary
              </h1>
              <p className="text-sm sm:text-base text-gray-600 mt-1">
                Registrations and participants marked non-pork, by status
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 bg-white text-gray-900 text-sm sm:text-base"
            >
              <option value="registration">Per Registration</option>
      ...
      {!loading && (totalRegistrations > 0 || totalNonPorkParticipants > 0) && (
        <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-500">Registrations with Non Pork</p>
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-500">Non Pork Participants</p>
                <p className="text-3xl font-bold text-gray-900">{totalNonPorkParticipants}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search by Reg ID, contact, province, LGU, or use regid:xxx"
      ...
      {loading ? (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-center">
          <p className="text-gray-500">Loading non-pork {viewLabel}...</p>
        </div>
      ) : !hasData ? (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-center">
          <p className="text-gray-500">No non-pork {viewLabel} found for this conference and status.</p>
        </div>
      ) : filteredCount === 0 ? (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-center">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1100px] table-fixed divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {(paginatedData as NonPorkRegistration[]).map((reg, index) => (
                  <tr key={`${reg.regid}-${index}`} className="group hover:bg-gray-50">
                    ...
                    <td className="px-2 py-2 whitespace-nowrap text-xs text-gray-700">{reg.status || 'N/A'}</td>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1100px] table-fixed divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {(paginatedData as NonPorkParticipant[]).map((p, index) => (
