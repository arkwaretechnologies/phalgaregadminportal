...
interface PendingLocationClientProps {
  initialConferences: Conference[];
  initialConfcode: string | null;
}

export default function PendingLocationClient({
  initialConferences,
  initialConfcode,
}: PendingLocationClientProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const url = new URL('/api/reports/pending-location', window.location.origin);
        ...
        if (!response.ok) {
          setError(data.error || 'Failed to fetch pending location data');
          setProvinceData([]);
          setLguData([]);
          setTotalPending(0);
          return;
        ...
        setError('An error occurred while fetching pending location data');
        setProvinceData([]);
        setLguData([]);
        setTotalPending(0);
      } finally {
  ...
  const handleConferenceChange = (confcode: string) => {
    ...
    router.push(`/dashboard/reports/pending-location?${params.toString()}`);
  };
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      if (activeTab === 'province') {
        ...
        const filename = `${safeConfName}_pending_by_province_${countBy}.xlsx`;
        ...
        const filename = `${safeConfName}_pending_by_lgu_${countBy}.xlsx`;
        XLSX.writeFile(wb, filename);
      }
    } catch (err: any) {
      console.error('Export error:', err);
      setError(err?.message || 'Failed to export pending location data');
    } finally {
      setExporting(false);
    }
  };
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base text-gray-600 mt-1">Pending registrations by Province and LGU</p>
            </div>
          </div>
      ...
      {selectedConference && (
        <div className="mb-6 p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl border border-amber-100">
          ...
          {selectedConference.name && (
            <p className="text-xs sm:text-sm text-amber-600 mt-1">{selectedConference.confcode}</p>
          )}
          {!loading && (
            <p className="text-sm text-amber-700 mt-2">
              Total Pending {countByLabel}: <span className="font-bold">{totalPending}</span>
            </p>
          )}
        </div>
      )}
      ...
      {loading ? (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sm:p-8 text-center">
          <p className="text-sm sm:text-base text-gray-500">Loading pending location data...</p>
        </div>
      ) : !hasData ? (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sm:p-8 text-center">
          <p className="text-sm sm:text-base text-gray-500">No pending {countBy === 'batch' ? 'registrations' : 'participants'} found for this conference.</p>
        </div>
      ) : (
        <>
          {/* Tab Navigation */}
