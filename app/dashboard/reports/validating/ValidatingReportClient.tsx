...
interface ValidatingRegistration {
  regid: string;
...
interface ValidatingParticipant {
  [key: string]: any;
  registration?: {
    regid: string;
    confcode: string | null;
...
interface ValidatingReportClientProps {
  initialConferences: Conference[];
  initialConfcode: string | null;
}

export default function ValidatingReportClient({
  initialConferences,
  initialConfcode,
}: ValidatingReportClientProps) {
  ...
  const [registrations, setRegistrations] = useState<ValidatingRegistration[]>([]);
  const [participants, setParticipants] = useState<ValidatingParticipant[]>([]);
  ...
  const [ancRegdParticipants, setAncRegdParticipants] = useState<ValidatingParticipant[]>([]);
  ...
  const matchesAncRegd = (
    reg: ValidatingRegistration,
    predicate: (p: ValidatingParticipant) => boolean
  ): boolean => {
  ...
  const participantFreeTextMatch = (reg: ValidatingRegistration, q: string): boolean =>
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const url = new URL('/api/reports/validating', window.location.origin);
        ...
        console.error('Error fetching validating report:', err);
  ...
  const handleConferenceChange = (confcode: string) => {
    ...
    router.push(`/dashboard/reports/validating?${params.toString()}`);
  };
    ...
    try {
      ...
      if (viewMode === 'registration') {
        ...
        const data = registrations.map((reg) => ({
          ...
          'Validation No.': reg.validation_no ?? 'N/A',
        ...
        ws['!cols'] = [
          { wch: 15 }, { wch: 14 }, { wch: 30 }, { wch: 25 }, { wch: 25 },
          { wch: 30 }, { wch: 18 }, { wch: 30 }, { wch: 18 },
        ];
        XLSX.utils.book_append_sheet(wb, ws, 'On Validation Regs');
        XLSX.writeFile(wb, `${safeConfName}_on_validation_registrations.xlsx`);
      } else {
        ...
        const data = participants.map((p) => {
          const row: Record<string, string | number> = {
            ...
            'Validation No.': p.registration?.validation_no ?? 'N/A',
        ...
        ws['!cols'] = isAncConf
          ? [
              { wch: 30 }, { wch: 45 }, { wch: 15 }, { wch: 14 }, { wch: 22 }, { wch: 22 },
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'On Validation Parts');
        XLSX.writeFile(wb, `${safeConfName}_on_validation_participants.xlsx`);
      }
    } catch (err: any) {
    ...
    <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base text-gray-600 mt-1">Registrations currently marked for validation</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-amber-500 focus:border-amber-500 bg-white text-gray-900 text-sm sm:text-base"
            >
              <option value="registration">On Validation Registrations</option>
              <option value="participant">On Validation Participants</option>
            </select>
          </div>
        </div>
      </div>
        ...
        <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-500">On Validation Registrations</p>
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-500">On Validation Participants</p>
      ...
      {loading ? (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-center">
          <p className="text-gray-500">Loading validating {viewLabel}...</p>
        </div>
      ) : !hasData ? (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-center">
          <p className="text-gray-500">No validating {viewLabel} found for this conference.</p>
        </div>
      ) : filteredCount === 0 ? (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-center">
            ...
            <table className="w-full min-w-[1100px] table-fixed divide-y divide-gray-200">
              ...
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-2 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Val. No.</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {(paginatedData as ValidatingRegistration[]).map((reg, index) => (
                  <tr key={`${reg.regid}-${index}`} className="group hover:bg-gray-50">
                    ...
                    <td className="px-2 py-2 whitespace-nowrap text-sm text-center tabular-nums text-gray-900">
                      {reg.validation_no ?? '—'}
                    </td>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1140px] table-fixed divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {(paginatedData as ValidatingParticipant[]).map((p, index) => (
